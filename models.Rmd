...
m_pooled <- lm(Length ~ TransectDepth, fishdata) 
...
df_pooled <- tibble(
  ...
  Slope_depth = coef(m_pooled)[2]
)
df_pooled


m_partialpooled <- lme(Length ~ TransectDepth, 
                        random = ~ 1 | CommonName, data = fishdata, method = "ML")
